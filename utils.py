...
def create_traffic_state(data, traffic_quantiles, feature="volumes_mean", mode="train"):
    if mode == "train":
        grouper = ["day", "t"]
    else:
        grouper = ["test_idx"]
    ...
    traffic_means["quantile"] = [(vol - traffic_quantiles).abs().argmin() for vol in traffic_means[feature]]
...
def load_supersegments(city_name, node_coordinates):
...
def create_nodes_with_counters(city_name, blacklist=False):
    ...
    if blacklist:
        ...
        # counters = load_counters(city_name, mode="train")
        # counters = counters.drop_duplicates(subset=["node_id"])
        # del counters["volumes_1h"]
        # del counters["day"]
        # del counters["t"]
        # counters["counter_id"] = list(range(len(counters)))
        # print(counters.shape)
